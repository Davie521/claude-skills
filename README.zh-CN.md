...
每个 skill 下面写清三件事：**定位**（它到底解决什么）、**触发**（怎么让它上场）、**机制**（它内部实际怎么跑）。
...
6. **Copilot 评论** —— `gh pr view --comments` **加上** `gh api repos/{owner}/{repo}/pulls/$PR_NUMBER/comments`；评论还没出现就等 30 秒重查 → 逐条评估 → 只修必要的 → 推送
7. **循环 4–6** 直到全部通过
...
> **为什么两个评论端点都要查**：Copilot 的行内 review 评论只存在于 REST 的 `repos/{owner}/{repo}/pulls/<n>/comments` 响应里，`gh pr view --comments` 只返回会话级评论 —— 只查后者会漏掉**全部**代码评论。Copilot 还是异步发评论的，所以有那条「等 30 秒重查」。
