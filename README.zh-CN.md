...
HEIC→PNG（`sips`）→ EXIF 旋转矫正 → 模型直接读图并翻译 → PIL + reportlab 排双语 PDF 输出。产出是**未认证草稿**：页脚与文件名都带该声明，需由人工复核并按目的国规则认证后才能提交，不能直接拿去交。
...
以文件内容的 SHA-256（再拼上处理器版本与配置）而不是路径做缓存键：文件移动改名后缓存仍命中，内容变了、或换了 OCR 引擎/模型/schema，都会自动失效；配合 service layer 分离，让缓存对业务代码完全透明。适合跨运行反复出现的重处理任务。
