...
Before running any script, make sure the virtual environment from SKILL.md's "Environment Setup" section is active (`pypdf`, `pdf2image` and `pdfplumber` installed via venv; never `--break-system-packages`).
...
**Optional visual check (Approach B only):** if fields.json uses `image_width`/`image_height`, you can render the boxes onto the page images and inspect them:
`python scripts/create_validation_image.py <page_number> fields.json <page_image.png> <validation_image.png>`

Entry boxes are drawn in red, label boxes in blue. Red rectangles must cover only input areas (never text), and for checkboxes must be centred on the square itself. This script assumes image pixel coordinates, so skip it when fields.json uses `pdf_width`/`pdf_height`.
